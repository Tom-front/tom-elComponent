<template>
  <div class="trend">
    ...
    <div class="icon">
      ...
      <component :is="`el-icon-${toLine(upIcon)}`" :style="{ color: reverseColor ? downIconColor : upIconColor }"
        v-if="type === 'up'"></component>
      <component :is="`el-icon-${toLine(downIcon)}`" :style="{ color: reverseColor ? upIconColor : downIconColor }" v-else>
      </component>
    </div>
  </div>
</template>
